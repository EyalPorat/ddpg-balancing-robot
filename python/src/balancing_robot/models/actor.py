...
class Actor(nn.Module):
    """Actor network for DDPG using delta-based control with time series state.
    ...
    def __init__(self, state_dim: int, action_dim: int, max_action: float, hidden_dims: Tuple[int, ...] = (64, 64)):
        """Initialize actor network for time series state.
        ...
        # Build network with larger dimensions for time series input
        layers = []
        prev_dim = state_dim  # Full time series dimension
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Process time series state directly
